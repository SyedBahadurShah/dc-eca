...
$sql = "
    SELECT *,
        club_invites.student_id AS 'invite_student_id',
        club_invites.status AS invite_status,
        club_memberships.status AS membership_status
    FROM clubs
    LEFT JOIN club_invites
        ON clubs.club_id = club_invites.club_id
        AND club_invites.student_id = :student_id
    LEFT JOIN club_memberships
        ON clubs.club_id = club_memberships.club_id
        AND club_memberships.student_id = :student_id
    LEFT JOIN club_categories
        ON clubs.category_id = club_categories.category_id
    WHERE clubs.club_id = :club_id
";

$statement = $pdo->prepare($sql);
$statement->bindParam(":club_id", $club_id);
$statement->bindParam(":student_id", $student_id); // Ensure student ID is used
$statement->execute();
...
if (empty($club["invite_status"]) && empty($club["membership_status"])) {
    ...
    <textarea name="application" class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
...
if (isset($_POST["apply"])) {
    ...
    if (empty($club["invite_status"]) && empty($club["membership_status"])) {
        ...
        $teacher_id = $club["created_by"];

        $sql = "INSERT INTO club_invites (club_id, application, student_id, teacher_id, status) VALUES (:club_id, :application, :student_id, :teacher_id, :status)";
        $statement = $pdo->prepare($sql);
        $statement->bindParam(":club_id", $club_id);
        $statement->bindParam(":student_id", $student_id);
        $statement->bindParam(":teacher_id", $teacher_id);
        $statement->bindParam(":application", $application);
