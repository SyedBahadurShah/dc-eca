...
$sql = "SELECT * FROM announcements WHERE recipient_type = 'all' OR recipient_type = 'student_leader' OR recipient_type = 'student' OR club_id IN(SELECT club_id FROM club_memberships WHERE student_id = :student_id AND status = :status) ORDER BY announcement_id DESC";
...
$sql = "
    ...
    ON
        c.club_id = cl.club_id
    LEFT JOIN users AS u
    ON
        u.user_id = cl.user_id
    LEFT JOIN users AS t
    ON
        t.user_id = c.created_by
    WHERE
        cl.user_id = :created_by OR c.created_by = :created_by
    ORDER BY
        cl.log_id
    DESC
";

// Fix this query (I need logs from my teachers)
...
foreach ($logs as $log) {
    ...
    if ($log["action_by"] == "teacher") {
        if ($log["action"] == "added") {
            ...
            $l['log'] = 'You were ' . '<strong><span class="text-danger">' . $log["action"] . '</span></strong> to <strong>' . $log["club_name"] . '</strong> club';
            $l['date'] = $log["date"];
            $logList[] = $l;
        }
